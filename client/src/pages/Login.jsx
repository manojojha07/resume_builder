...
const navigate = useNavigate();
  ...
  const urlState = query.get('state')
   ...
   const [state, setState] = React.useState(urlState || "login")
    ...
    e.preventDefault();
    try {
        const { data } = await api.post(`/api/auth/${state}`, formData);
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="sm:w-[450px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                <div 
                className="flex items-center w-full mt-4 bg-white 
                border border-gray-300/80 h-12 rounded-lg overflow-hidden pl-6 gap-2">
                  <Mail  size={16} color='#6B7280'/>
                  <input type="email" name="email" placeholder="Email id" 
                  className="border-none outline-none ring-0" value={formData.email}
                   onChange={handleChange} required />
                </div>
                <div className="flex items-center mt-4 w-full bg-white border
                 border-gray-300/80 h-12 rounded-lg overflow-hidden pl-6 gap-2">
